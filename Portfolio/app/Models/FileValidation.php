...
class FileValidation {
    ...
    private function required($field): void
    {
        if (!$this->fieldExists($field)) {
            throw new Exception("❌ Das {$field} Feld darf nicht leer sein.");
        }
    }
    ...
    private function type($field, $satisfier): void
    {
        ...
        if (!in_array($extension, $allowedExtensions)) {
            throw new Exception("❌ Das {$field} Feld muss vom Dateitypen {$satisfier} sein.");
        }
        ...
        if ($mimeType !== $allowedMimeType) {
            throw new Exception("❌ Das {$field} Feld muss vom Typen {$satisfier} sein.");
        }
    }
    ...
    private function maxsize($field, $satisfier)
    {
        if ($this->inputFiles[$field]['size'] > (int) $satisfier) {
            throw new Exception("❌ Das {$field} Feld darf nicht länger als {$satisfier} bytes sein.");
        }
    }
}